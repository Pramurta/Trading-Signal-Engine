...
def demo_signals(data: dict):
    ...
    print(
        f"  Z-Score:    {zscore.get_current():+.3f}  (strength: {zscore.strength:.2f})"
    )
    print(f"  RSI:        {rsi.get_current():+.3f}  (strength: {rsi.strength:.2f})")
    macd_val = macd.get_current()
    print(f"  MACD:       {macd_val:+.3f}  (strength: {macd.strength:.2f})")
    boll_val = bollinger.get_current()
    print(f"  Bollinger:  {boll_val:+.3f}  (strength: {bollinger.strength:.2f})")
    print("  ─────────────────────────────")
    comb_val = combined.get_current()
    print(f"  Combined:   {comb_val:+.3f}  (strength: {combined.strength:.2f})")
...
def demo_risk_management(data: dict):
    ...
    print("  Signal Strength: 0.75")
    print("  Win Rate: 55%")
    print("  Win/Loss Ratio: 1.5x")
...
async def demo_full_backtest():
    ...
    print("Transaction Cost: 5 bps")

    results = await strategy.run_backtest(
        symbols=symbols, days=252, initial_capital=100_000
    )
