...
from src.data_handler import MarketDataHandler, DataSource
    ...
    def test_strategy_backtest(self):
        ...
        async def run_backtest():
            strategy = TradingStrategy(
                data_handler=MarketDataHandler(source=DataSource.SYNTHETIC, random_seed=42),
...
def run_verification():
    ...
    tests = [
        ...
        ("Data Handler - OHLCV Fields", TestDataHandler().test_data_has_all_ohlcv_fields),
        ("Data Handler - OHLC Consistency", TestDataHandler().test_ohlc_consistency),
        ("Pipeline - Signals", lambda: TestFullPipeline().test_signals_from_real_prices(TestFullPipeline().sample_data.fget(None))),
