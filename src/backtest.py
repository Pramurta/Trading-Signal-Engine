...
class Backtester:
    ...
    def run(
        ...
        if signals.shape != prices.shape:
            raise ValueError(f"Signal shape {signals.shape} != price shape {prices.shape}")
        ...
        portfolio_returns = np.sum(lagged_positions * returns, axis=1) - transaction_costs
    ...
    def _calculate_metrics(
        ...
        if len(downside_returns) > 0:
            ...
            sortino = (np.mean(excess_returns) * 252 / downside_vol) if downside_vol > 0 else 0
        else:
            sortino = 0
