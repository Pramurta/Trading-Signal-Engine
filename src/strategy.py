...
from .risk_manager import Portfolio, PositionSize, RiskManager, RiskMetrics
...
class TradingStrategy:
                ...
                if day % rebalance_frequency == 0:
                    ...
                    if target_pos != current_pos:
                        ...
                        if target_pos != 0 and current_pos == 0:
                            ...
                            trade_pnl = (
                                current_price - entry_prices[symbol]
                            ) * current_pos
                            if trade_pnl > 0:
                                wins += 1
    ...
    def print_backtest_results(self, results: BacktestResult) -> str:
        ...
        report.append(
            f"  Current Drawdown:  {results.risk_metrics.current_drawdown:>10.2%}"
        )
        report.append(
            f"  Risk Level:        {results.risk_metrics.risk_level.value:>10s}"
        )
        report.append("")
        report.append("=" * 70)
...
async def example_strategy():
    ...
    results = await strategy.run_backtest(
        symbols=["AAPL", "GOOGL", "MSFT", "AMZN"], days=252, initial_capital=100_000
    )
